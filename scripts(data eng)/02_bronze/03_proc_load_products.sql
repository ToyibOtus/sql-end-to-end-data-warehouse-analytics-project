...
=============================================================================================
Script Purpose:
	This script loads data from a CSV file into a bronze table [products]. Additionaly, 
	it loads log tables with vital log details, which is essential for easy traceability 
	and debugging.

Parameter: @job_run_id

Usage: EXEC bronze.usp_load_bronze_products @job_run_id
...
BEGIN
	...
	DECLARE
	@step_run_id INT,
	@step_name NVARCHAR(50) = 'bronze.usp_load_bronze_products',
	...
	BEGIN TRY
		...
		SET @sql = 'BULK INSERT bronze.products FROM ''' + @source_path + ''' WITH (FIRSTROW = 2, FIELDTERMINATOR = '';'', TABLOCK);';
		EXEC (@sql);
