...
BEGIN
	...
	DECLARE
	@step_run_id INT,
	@step_name NVARCHAR(50) = 'silver.usp_load_silver_products',
	...
	@source_path NVARCHAR(50) = 'bronze.products',
	@rows_in_source INT,
	@rows_to_load INT,
	@rows_updated INT = 0,
	@rows_inserted INT = 0,
	...
	SELECT @rows_in_source = COUNT(*) FROM bronze.products;
	...
	BEGIN TRY
		...
		TRUNCATE TABLE silver_stg.products;
		...
		SELECT
			...
			COALESCE(UPPER(product_name), ''),
			COALESCE(UPPER(category), ''),
			COALESCE(UPPER(sub_category), ''))) AS dwh_row_hash
		FROM data_transformations
		)
		-- Load staging table
		INSERT INTO silver_stg.products
		(
			product_id,
			product_name,
			category,
		...
		ON mc.product_id = sp.product_id
		AND mc.dwh_row_hash = sp.dwh_row_hash
		WHERE sp.dwh_row_hash IS NULL;

		-- Retrieve rows to be loaded
		SELECT @rows_to_load = COUNT(*) FROM silver_stg.products;
		...
		SELECT
			...
			SUM(CASE WHEN product_id IS NULL THEN 1 ELSE 0 END) AS rows_failed_pk_null,
			COUNT(product_id) - COUNT(DISTINCT product_id) AS rows_failed_pk_duplicate,
			SUM(CASE WHEN category NOT IN ('Office Supplies', 'Furniture', 'Technology') THEN 1 ELSE 0 END) AS rows_failed_invalid_category
			INTO #dq_metrics
		FROM silver_stg.products;
		...
		UNION ALL
		SELECT @job_run_id, @step_run_id, @ingest_layer, @ingest_table, 'INVALID_CATEGORY', rows_checked, rows_failed_invalid_category AS rows_failed,
		CASE WHEN rows_failed_invalid_category > 0 THEN 'FAILED' ELSE 'PASSED' END AS dq_status FROM #dq_metrics;
		...
		AND (dq_check_name = 'PK_NULL' OR dq_check_name = 'PK_DUPLICATE') AND (rows_failed > 0)) THROW 50003, 
		'Critical Data Quality Rule Violated: Unable to load due to PRIMARY KEY containing either NULLS or DUPLICATES.', 3;
		...
		SELECT
			...
			src.sub_category,
			src.dwh_row_hash
		FROM silver_stg.products src
		LEFT JOIN silver.products tgt
